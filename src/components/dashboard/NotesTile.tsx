...
export const NotesTile = ({ project }: NotesTileProps) => {
  ...
  return (
    <div className="h-full max-w-[851px]">
      {/* Wrapper div to contain the shadow overflow */}
      <div className="h-full p-6 rounded-[20px] bg-background border-[0.5px] border-[#CCCCCC] dark:border-[#5E6664] shadow-[5px_20px_20px_rgba(0,0,0,0.1)] dark:shadow-[5px_20px_20px_rgba(255,255,255,0.05)] flex flex-col">
        <Tabs 
          value={activeTab} 
          onValueChange={setActiveTab}
          className="flex-1 flex flex-col h-full"
        >
          <TabsList className="flex w-full mb-4 bg-transparent p-0 justify-start gap-6 shadow-none !shadow-none [--tw-shadow:none]">
            <TabsTrigger 
